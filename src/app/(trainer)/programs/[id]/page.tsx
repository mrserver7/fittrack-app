  ...
  return (
    <div className="page-container">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            ...
            {program.goalTag && (
              <span className="text-xs bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 px-2.5 py-1 rounded-full font-medium">{program.goalTag}</span>
            )}
          </div>
        ...
        <div className="flex items-center gap-2">
          <DeleteProgramButton
            ...
            activeClientCount={program.clientPrograms.length}
          />
      ...
      {program.clientPrograms.length > 0 && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-2xl p-4 mb-6 flex items-center gap-3">
      ...
      <div className="space-y-6">
        {program.weeks.map((week) => (
          <div key={week.id} className="section-card overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-border bg-muted">
              <div className="flex items-center gap-3">
                <ClipboardList className="w-4 h-4 text-muted-foreground" />
                <h2 className="font-semibold text-foreground">Week {week.weekNumber}</h2>
                {week.isDeload && (
                  <span className="text-xs bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-300 px-2.5 py-1 rounded-full font-medium">Deload</span>
                )}
              </div>
              <span className="text-xs text-muted-foreground">{week.days.length} day{week.days.length !== 1 ? "s" : ""}</span>
            </div>
              ...
              {week.days.map((day) => (
                <div key={day.id} className="border border-border rounded-xl overflow-hidden">
                  <div className="bg-muted px-4 py-2.5 border-b border-border">
                    <h3 className="font-semibold text-foreground text-sm">{day.dayLabel}</h3>
                  ...
                  <div className="p-3 space-y-2">
                    {day.exercises.length === 0 ? (
                      ...
                      day.exercises.map((ex, idx) => (
                        <div key={ex.id} className="flex items-start gap-2 p-2.5 rounded-lg bg-muted">
                          <span className="text-xs font-bold text-muted-foreground/50 w-5 flex-shrink-0 mt-0.5">{idx + 1}</span>
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-foreground truncate">{ex.exercise.name}</p>
                            <p className="text-xs text-muted-foreground">
                              {ex.sets}x{ex.repsMin}{ex.repsMax && ex.repsMax !== ex.repsMin ? `--${ex.repsMax}` : ""} reps
                              {ex.rpeMin ? ` / RPE ${ex.rpeMin}${ex.rpeMax ? `--${ex.rpeMax}` : ""}` : ""}
                              {ex.restSeconds ? ` / ${ex.restSeconds}s rest` : ""}
                            </p>
                            {ex.coachingNote && (
                              <p className="text-xs text-blue-600 dark:text-blue-400 mt-0.5 truncate">{ex.coachingNote}</p>
                            )}
