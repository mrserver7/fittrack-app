  ...
  return (
    <div className="p-6 md:p-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-50">Workout History</h1>
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">{sessions.length} session{sessions.length !== 1 ? "s" : ""} logged</p>
        </div>
        {activeProgram && (
          <Link href="/workout/today"
            ...
            <Zap className="w-4 h-4" /> Today&apos;s Workout
          </Link>
        )}
      </div>
      ...
      {sessions.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-gray-900 rounded-2xl border border-dashed border-gray-300 dark:border-gray-700">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-50 mb-2">No sessions yet</h3>
          <p className="text-gray-500 dark:text-gray-400 text-sm">Complete your first workout to see it here.</p>
          {activeProgram && (
            <Link href="/workout/today"
            ...
            <Link key={s.id} href={`/workouts/${s.id}`}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                  {s.workoutDay?.dayLabel || "Free session"}
