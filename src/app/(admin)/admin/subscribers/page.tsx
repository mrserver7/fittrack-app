  ...
  return (
    <div className="page-container">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">{t.admin.allSubscribers}</h1>
          ...
          <Link
            ...
            className={`px-3 py-2 rounded-xl text-xs font-medium border transition-colors flex items-center gap-1.5 ${
              status === s
                ...
                : "bg-card text-muted-foreground border-border hover:border-gray-300 dark:hover:border-gray-600"
            }`}>
      ...
      {allSubscribers.length === 0 ? (
        <div className="text-center py-20 section-card border-dashed">
          ...
          <h3 className="text-lg font-semibold text-foreground mb-2">{t.admin.noSubscribersFound}</h3>
          <p className="text-muted-foreground text-sm">{t.admin.tryAdjustingFilter}</p>
        </div>
      ) : (
        <div className="space-y-3">
          {allSubscribers.map((client) => (
            <div
              key={client.id}
              className={`section-card p-5 flex items-center gap-4 ${
                client.status === "pending" ? "!border-orange-200 dark:!border-orange-800" : ""
              }`}>
              ...
              <div className="flex items-center gap-2 flex-shrink-0 flex-wrap justify-end">
                ...
                <Link href={`/admin/subscribers/${client.id}`}
                  className="px-3 py-1.5 text-xs font-medium border border-border rounded-lg text-muted-foreground hover:bg-muted/60 transition-colors whitespace-nowrap">
                  View →
                </Link>
